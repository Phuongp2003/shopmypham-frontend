    ...
    >
      <template #header>
        <div class="flex flex-col items-center gap-2">
          ...
          <h2 class="text-xl font-semibold mt-2 text-gray-900 dark:text-white">{{ user?.name }}</h2>
        </div>
      </template>
      ...
      <div v-else class="flex flex-col gap-4 py-4 w-full">
        <div class="flex flex-col gap-2">
          ...
          <div class="flex items-center justify-between">
            <span class="font-medium text-gray-700 dark:text-gray-200">Số điện thoại:</span>
            <span class="text-gray-900 dark:text-white">{{ user?.phone || 'Chưa cập nhật' }}</span>
          </div>
          <div class="flex items-center justify-between">
          ...
          <div class="flex items-center justify-between">
            <span class="font-medium text-gray-700 dark:text-gray-200">Trạng thái:</span>
          ...
          <div class="flex items-center justify-between">
            <span class="font-medium text-gray-700 dark:text-gray-200">Ngày cập nhật:</span>
            <span class="text-gray-900 dark:text-white">{{
          ...
          <div class="flex items-center justify-between">
            ...
            <span class="text-gray-900 dark:text-white">
              <template v-if="user?.googleId">
                Đã liên kết với <span class="font-semibold">{{ user?.googleEmail }}</span>
                <UButton
                  size="xs"
                  color="error"
                  variant="soft"
...
const { user, isLoading, isError, error, getUserProfile, unlinkGoogle } =
  useUserProfile()
