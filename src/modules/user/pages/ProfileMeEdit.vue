<template>
  <div class="flex flex-col md:flex-row gap-8 justify-center items-start min-h-[60vh] bg-gray-100 dark:bg-gray-900 py-8">
    <!-- Profile Edit Card -->
    <UCard class="w-full max-w-xl shadow-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
      <template #header>
        <div class="flex flex-col items-center gap-2">
          <UserAvatar :userName="user?.name || 'User'" />
          <h2 class="text-xl font-semibold mt-2 text-gray-900 dark:text-white">Chỉnh sửa thông tin cá nhân</h2>
        </div>
      </template>
      <div v-if="isLoading" class="text-center py-8 text-gray-700 dark:text-gray-200">Loading...</div>
      ...
      <div v-else>
        <UForm :schema="profileSchema" :state="profileState" class="space-y-6" @submit="onProfileSubmit">
          ...
          <div class="flex justify-end">
            <UButton type="submit" color="primary" class="font-semibold shadow-md">Lưu thay đổi</UButton>
          </div>
        </UForm>
      </div>
    </UCard>
    ...
    <UCard class="w-full max-w-xl shadow-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
      <template #header>
        <div class="flex flex-col items-center gap-2">
          <h2 class="text-xl font-semibold mt-2 text-gray-900 dark:text-white">Đổi mật khẩu</h2>
        </div>
      </template>
      <UForm :schema="passwordSchema" :state="passwordState" class="space-y-6">
        <UFormField label="Mật khẩu cũ" name="oldPassword" v-if="!user?.isNoPassword">
          <UInput v-model="passwordState.oldPassword" type="password" class="w-full" placeholder="Nhập mật khẩu cũ" />
        </UFormField>
        <UFormField label="Mật khẩu mới" name="newPassword">
          <UInput v-model="passwordState.newPassword" type="password" class="w-full" placeholder="Nhập mật khẩu mới" />
        </UFormField>
        <UFormField label="Nhập lại mật khẩu mới" name="confirmNewPassword">
          <UInput v-model="passwordState.confirmNewPassword" type="password" class="w-full" placeholder="Nhập lại mật khẩu mới" />
        </UFormField>
        <UFormField label="OTP" name="otp">
          <div class="flex gap-2">
            <UInput v-model="passwordState.otp" type="text" class="w-full" placeholder="Nhập mã OTP" />
            <UButton type="button" color="primary" @click="onRequestOTP" :loading="otpLoading">Nhận OTP</UButton>
          </div>
        </UFormField>
        <div class="flex justify-end">
          <UButton type="submit" color="primary" class="font-semibold shadow-md" @click="onPasswordSubmit" :loading="passwordLoading">Đổi mật khẩu</UButton>
        </div>
      </UForm>
    </UCard>
  </div>
...
async function onProfileSubmit() {
  try {
    ...
    toast.add({ title: 'Thành công', description: 'Cập nhật thông tin thành công.', color: 'success' })
    await getUserProfile()
  } catch (err) {
...
async function onRequestOTP() {
  if (!authStore.user?.email) {
    toast.add({ title: 'Lỗi', description: 'Không tìm thấy email người dùng.', color: 'error' })
    return
  }
  otpLoading.value = true
  try {
    await generateOTPKeyApi({ email: authStore.user.email })
    toast.add({ title: 'Thành công', description: 'OTP đã được gửi về email của bạn.', color: 'success' })
  } catch (err) {
...
async function onPasswordSubmit() {
  ...
  try {
    ...
    toast.add({ title: 'Thành công', description: 'Đổi mật khẩu thành công.', color: 'success' })
