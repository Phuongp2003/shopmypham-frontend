...
export const getUsersApi = async (
    ...
    const response = await api.get<PaginationUserRes>('/users', {
        params: {
            page,
            limit,
            q: search,
...
export const getUserByIdApi = async (id: string): Promise<GetUserRes> => {
    const response = await api.get<GetUserRes>(`/users/${id}`)
        .catch((err) => {
            throw new Error(err.response.data.message);
        });
    return response.data;
...
export const createUserApi = async (data: CreateUserReq) => {
    const response = await api.post<GetUserRes>('/users', data)
        .catch((err) => {
            throw new Error(err.response.data.message);
        });
    return response.data;
...
export const updateUserApi = async (id: string, data: UpdateUserReq) => {
    const response = await api.put<GetUserRes>(`/users/${id}`, data)
        .catch((err) => {
            throw new Error(err.response.data.message);
        });
    return response.data;
...
export const changeUserStatusApi = async (id: string, status: UserStatus) => {
    const response = await api.patch<GetUserRes>(`/users/${id}/status`, { status })
        .catch((err) => {
            throw new Error(err.response.data.message);
        });
    return response.data;
