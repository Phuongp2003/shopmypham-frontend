  ...
  <div class="mt-12">
    <UTabs :items="tabs" v-model="currentTab">
      <template #content="{ item }">
        <div class="mt-2 shadow-lg shadow-primary-500/25 rounded-lg p-4 bg-soft-elevated border border-gray-200 dark:border-gray-700">
          <!-- Description Tab -->
          ...
          <!-- Application Tab -->
          <ApplicationTab v-else-if="item.value === 'application'" :usageInstructions="cosmetic?.usageInstructions" />

          <!-- Shipping Tab -->
          <ShippingTab v-else-if="item.value === 'shipping'" :shippingPolicy="cosmetic?.shippingPolicy" />

          <!-- Reviews Tab -->
          <ReviewsTab v-else-if="item.value === 'reviews'"
...
const sortedBenefits = computed(() => {
  ...
  return props.cosmetic.benefits
