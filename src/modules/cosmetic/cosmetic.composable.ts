...
interface GetCosmeticsParams {
  ...
  limit?: number
  search?: string
  sortBy?: 'price' | 'name' | 'createdAt' | 'latest' | 'popular' | 'price_asc' | 'price_desc' | 'rating_desc' | 'name_asc' | 'name_desc'
  priceRange?: string
  minRating?: string
}
  ...
  const getCosmetics = async (params: GetCosmeticsParams = {}) => {
    try {
      ...
      if (params.priceRange) {
        const [min, max] = params.priceRange.split('-').map(p => p ? parseInt(p) : undefined)
